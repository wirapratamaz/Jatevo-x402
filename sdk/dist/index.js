...
__export(index_exports, {
  ...
  SolanaPaymentHandler: () => SolanaPaymentHandler,
...
var SolanaPaymentHandler = class {
  ...
  constructor(config) {
    ...
    this.connection = new import_web3.Connection(config?.rpcUrl || configData.rpcUrl, "confirmed");
    this.usdcMint = new import_web3.PublicKey(configData.usdcMint);
  }
  /**
  ...
  async createPayment(amount, receiverAddress, wallet) {
    ...
    const senderPublicKey = activeWallet.publicKey || (activeWallet instanceof import_web3.Keypair ? activeWallet.publicKey : new import_web3.PublicKey(activeWallet));
    const amountMicroUsdc = parseInt(amount);
    const receiverPubkey = new import_web3.PublicKey(receiverAddress);
    const senderTokenAccount = await (0, import_spl_token.getAssociatedTokenAddress)(
      this.usdcMint,
      senderPublicKey
    );
    const receiverTokenAccount = await (0, import_spl_token.getAssociatedTokenAddress)(
      this.usdcMint,
      receiverPubkey
    );
    try {
      const senderAccount = await (0, import_spl_token.getAccount)(this.connection, senderTokenAccount);
    ...
    try {
      await (0, import_spl_token.getAccount)(this.connection, receiverTokenAccount);
    } catch (error) {
      if (error instanceof import_spl_token.TokenAccountNotFoundError) {
        const createATAInstruction = (0, import_spl_token.createAssociatedTokenAccountInstruction)(
          senderPublicKey,
    ...
    } else if (activeWallet instanceof import_web3.Keypair) {
...
var PaymentHandler = class {
  constructor(config) {
    ...
    if (config?.privateKey && !isSolanaNetwork(this.network)) {
      this.evmAccount = (0, import_accounts.privateKeyToAccount)(config.privateKey);
    }
